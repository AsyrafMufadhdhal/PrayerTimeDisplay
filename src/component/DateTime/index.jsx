...
const DateTime = ({ date }) => {
  ...
  return (
    <div className="row px-5">
      ...
      <div className={`col-auto rounded-start-pill px-5 d-flex align-items-center ${isFinished && "d-none"} `} id={styles.nextName}>
        <p className="fw-semibold m-0 rounded-pill ">
          Menuju Waktu <span id={styles.nextTitle}> {now == "terbit" ? (now = "syuruq") : now}</span>
        </p>
      </div>
      <div className={`col-2 text-center p-0 rounded-end-pill ${isFinished && "d-none"}`} id={styles.nextTime}>
